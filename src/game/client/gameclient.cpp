...
void CGameClient::OnConsoleInit()
{
	...
	m_vpAll.insert(m_vpAll.end(), {&m_Skins,
					      ...
					      &m_Mod, // TClient
					      &m_AvoidFreeze, // TClient - Avoiding Freeze indicator
					      &m_Bot, // TClient - AI Speedrun Bot
					      &m_TasBot, // TClient - TAS AI
...
void CGameClient::OnInit()
{
	...
	{
		// Spoofer: use DDNet or TClient identity
		if(g_Config.m_TcSpooferClient == 0) // DDNet
		{
			if(GIT_SHORTREV_HASH)
				str_format(m_aDDNetVersionStr, sizeof(m_aDDNetVersionStr), "DDNet %s (%s)", GAME_RELEASE_VERSION, GIT_SHORTREV_HASH);
			else
				str_format(m_aDDNetVersionStr, sizeof(m_aDDNetVersionStr), "DDNet %s", GAME_RELEASE_VERSION);
		}
		else // TClient (normal)
		{
			if(GIT_SHORTREV_HASH)
...
void CGameClient::OnMessage(int MsgId, CUnpacker *pUnpacker, int Conn, bool Dummy)
{
	...
	else if(MsgId == NETMSGTYPE_SV_KILLMSG)
	{
		...
		// TClient AI Learning
		if (pMsg->m_Victim == m_Snap.m_LocalClientId || (Client()->DummyConnected() && pMsg->m_Victim == m_aLocalIds[!g_Config.m_ClDummy]))
		{
			m_AvoidFreeze.UpdateLearning(true, false);
		}
