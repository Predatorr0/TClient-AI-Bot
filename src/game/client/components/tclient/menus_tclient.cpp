...
void CMenus::RenderSettingsTClientSettings(CUIRect MainView)
{
	...
	s_SectionBoxes.push_back(Column);
	Column.HSplitTop(HeadlineHeight, &Label, &Column);
	Ui()->DoLabel(&Label, TCLocalize("Avoiding Freeze"), HeadlineFontSize, TEXTALIGN_ML);
	Column.HSplitTop(MarginSmall, nullptr, &Column);

	{
		CUIRect CheckBoxRect;
		Column.HSplitTop(LineSize, &CheckBoxRect, &Column);
		if(DoButton_CheckBox(&g_Config.m_TcAvoidFreeze, TCLocalize("Enable Avoiding Freeze"), g_Config.m_TcAvoidFreeze != 0, &CheckBoxRect))
			g_Config.m_TcAvoidFreeze = g_Config.m_TcAvoidFreeze ? 0 : 1;
	}

	if(g_Config.m_TcAvoidFreeze)
	{
		{
			static std::vector<CButtonContainer> s_vModeButtons = {{}, {}};
			int Mode = g_Config.m_TcAvoidFreeze == 2 ? 1 : 0;
			if(DoLine_RadioMenu(Column, TCLocalize("Mode"), s_vModeButtons, {TCLocalize("Legit"), TCLocalize("Rage")}, {0, 1}, Mode))
				g_Config.m_TcAvoidFreeze = Mode == 1 ? 2 : 1;
		}

		DoButton_CheckBoxAutoVMarginAndSet(&g_Config.m_TcAvoidFreezeAuto, TCLocalize("Adaptive Lookahead (Ping-based)"), &g_Config.m_TcAvoidFreezeAuto, &Column, LineSize);
		if(!g_Config.m_TcAvoidFreezeAuto)
		{
			Column.HSplitTop(LineSize, &Button, &Column);
			Ui()->DoScrollbarOption(&g_Config.m_TcAvoidFreezeTicks, &g_Config.m_TcAvoidFreezeTicks, &Button, TCLocalize("Manual Lookahead"), 2, 15, &CUi::ms_LinearScrollbarScale, 0, "ticks");
		}

		Column.HSplitTop(LineSize, &Button, &Column);
		Ui()->DoScrollbarOption(&g_Config.m_TcAvoidFreezeMargin, &g_Config.m_TcAvoidFreezeMargin, &Button, TCLocalize("Safety Margin"), 0, 200);

		DoButton_CheckBoxAutoVMarginAndSet(&g_Config.m_TcAvoidFreezeDeath, TCLocalize("Avoid Death Tiles"), &g_Config.m_TcAvoidFreezeDeath, &Column, LineSize);
		DoButton_CheckBoxAutoVMarginAndSet(&g_Config.m_TcAvoidFreezeTele, TCLocalize("Avoid Teleport Tiles (Freeze)"), &g_Config.m_TcAvoidFreezeTele, &Column, LineSize);
		DoButton_CheckBoxAutoVMarginAndSet(&g_Config.m_TcAvoidFreezeLaser, TCLocalize("Avoid Laser/Plasma Projectiles"), &g_Config.m_TcAvoidFreezeLaser, &Column, LineSize);
		DoButton_CheckBoxAutoVMarginAndSet(&g_Config.m_TcAvoidFreezeSmartHook, TCLocalize("Smart Hook Blocking"), &g_Config.m_TcAvoidFreezeSmartHook, &Column, LineSize);
		DoButton_CheckBoxAutoVMarginAndSet(&g_Config.m_TcAvoidFreezeSmartJump, TCLocalize("Smart Jump/AD Blocking"), &g_Config.m_TcAvoidFreezeSmartJump, &Column, LineSize);
	}
	s_SectionBoxes.back().h = Column.y - s_SectionBoxes.back().y;
	Column.HSplitTop(MarginSmall, nullptr, &Column);
	...
	s_SectionBoxes.push_back(Column);
	Column.HSplitTop(HeadlineHeight, &Label, &Column);
	Ui()->DoLabel(&Label, TCLocalize("TAS AI Bot (DDNet TAS)"), HeadlineFontSize, TEXTALIGN_ML);
	Column.HSplitTop(MarginSmall, nullptr, &Column);

	DoButton_CheckBoxAutoVMarginAndSet(&g_Config.m_ClTasTraining, TCLocalize("Training Mode (Learn Map)"), &g_Config.m_ClTasTraining, &Column, LineSize);
	...
	{
		static CButtonContainer s_PlayTasButton;
		if(DoButton_Menu(&s_PlayTasButton, TCLocalize("Play Master Run"), 0, &Button))
			this->GameClient()->m_TasBot.StartPlayback();
	}
	else
	{
		Ui()->DoLabel(&Button, TCLocalize("No Master Run found for this map"), FontSize * 0.8f, TEXTALIGN_ML);
	}
	...
	// ***** AI Speedrun Bot (Legacy) ***** //
	Column.HSplitTop(MarginBetweenSections, nullptr, &Column);
	s_SectionBoxes.push_back(Column);
	Column.HSplitTop(HeadlineHeight, &Label, &Column);
	Ui()->DoLabel(&Label, TCLocalize("AI Speedrun Bot (Pathfinder)"), HeadlineFontSize, TEXTALIGN_ML);
	Column.HSplitTop(MarginSmall, nullptr, &Column);

	DoButton_CheckBoxAutoVMarginAndSet(&g_Config.m_TcBotEnabled, TCLocalize("Enable AI Bot"), &g_Config.m_TcBotEnabled, &Column, LineSize);
	DoButton_CheckBoxAutoVMarginAndSet(&g_Config.m_TcBotNavigation, TCLocalize("Enable Pathfinding (Navigation)"), &g_Config.m_TcBotNavigation, &Column, LineSize);
	...
	s_SectionBoxes.push_back(Column);
	Column.HSplitTop(HeadlineHeight, &Label, &Column);
	Ui()->DoLabel(&Label, TCLocalize("Client Spoofer"), HeadlineFontSize, TEXTALIGN_ML);
	Column.HSplitTop(MarginSmall, nullptr, &Column);

	DoButton_CheckBoxAutoVMarginAndSet(&g_Config.m_TcSpoofer, TCLocalize("Enable Spoofer"), &g_Config.m_TcSpoofer, &Column, LineSize);
	if(g_Config.m_TcSpoofer)
	{
		static std::vector<CButtonContainer> s_vSpooferButtons = {{}, {}};
		int ClientIdent = g_Config.m_TcSpooferClient;
		if(DoLine_RadioMenu(Column, TCLocalize("Spoof Identity"), s_vSpooferButtons, {TCLocalize("DDNet"), TCLocalize("TClient")}, {0, 1}, ClientIdent))
			g_Config.m_TcSpooferClient = ClientIdent;

		Column.HSplitTop(LineSize, &Label, &Column);
		Ui()->DoLabel(&Label, TCLocalize("Requires reconnect to take effect"), StandardFontSize * 0.8f, TEXTALIGN_ML);
	}
	s_SectionBoxes.back().h = Column.y - s_SectionBoxes.back().y;
	Column.HSplitTop(MarginSmall, nullptr, &Column);
